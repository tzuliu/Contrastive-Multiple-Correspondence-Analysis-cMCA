...
handles = [handles[0], handles[1], handles[2], handles[5], handles[7], handles[9], handles[8], handles[6], handles[4], handles[3]]
labels = [labels[0], labels[1], labels[2], labels[5], labels[7], labels[9], labels[8], labels[6], labels[4], labels[3]]
plt.xlabel('cPC1')
plt.ylabel('cPC2')
plt.legend(handles, labels, loc='best', shadow=False, scatterpoints=1, fontsize=8)
plt.show()
f.savefig("cMCA_CCES2015_repdem_loading_1.pdf", bbox_inches='tight')
