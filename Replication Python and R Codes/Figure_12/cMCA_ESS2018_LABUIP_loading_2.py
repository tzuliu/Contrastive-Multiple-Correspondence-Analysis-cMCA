...
handles = [handles[2], handles[3], handles[4], handles[9], handles[8], handles[6], handles[7], handles[5], handles[1], handles[0]]
labels = [labels[2], labels[3], labels[4], labels[9], labels[8], labels[6], labels[7], labels[5], labels[1], labels[0]]
plt.xlabel('cPC1')
plt.ylabel('cPC2')
plt.legend(handles, labels, loc='best', shadow=False, scatterpoints=1, fontsize=8)
plt.show()
f.savefig("cMCA_ESS2018_labuip_loading_2.pdf", bbox_inches='tight')
