...
dem_lib = list()
for t,u,v in zip(X_d['militarism.strength'],X_d['militarism.diplomacy'],X_d['egalitarianism.notbigprob']):
...
plt.scatter(Y_bg[:, 0], Y_bg[:, 1], c=tableau10[X_r["partyid"].iloc[0]], label=party[X_r["partyid"].iloc[0]], alpha=0.4, linewidths=0)
...
plt.show()
f.savefig("cMCA_CCES2015_demrep_new.pdf", bbox_inches='tight')
