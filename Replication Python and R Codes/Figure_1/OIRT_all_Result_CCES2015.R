...
test <- MCMCordfactanal(issuesvalues, factors=2, lambda.constraints=list(ideology=list(2,"+")),
...
irtdt_n <- irtdt %>% filter(party == "DEM" | party == "REP")

pdf("cces2015_oirt.pdf", width=5.8, height=4.6)
ggplot(data=irtdt_n, aes(x=coord1D, y=coord2D, color=party)) + geom_point(size=2, alpha=0.6) +
  #xlim(-1,1) +
  #ylim(-1,1) +
  xlab("\nPC1") +
  ylab("PC2\n") +
  ggtitle("Ordinal IRT Result of CCES 2015") +
