...
pro_eu = list()
for u,v in zip(X_lab['imwbcnt'],X_lab['atcherp']):
    if u==5 or v==5:# or u==4 or v==4:
        pro_eu.append(4)
    else:
        pro_eu.append(2)
...
plt.xlim([-1.5, 6])
plt.ylim([-1.5, 1.5])
for label in X_lab['pro_eu'].unique():
    plt.scatter(Y_fg[X_lab['pro_eu'] == label, 0], Y_fg[X_lab['pro_eu'] == label, 1], c=tableau10[label], label=label, alpha=0.6, linewidths=0)
#plt.scatter(Y_bg[:, 0], Y_bg[:, 1], c=tableau10[5], label=party[X_uip["prtclcgb"].iloc[0]], alpha=0.8, linewidths=0)
handles, labels = plt.gca().get_legend_handles_labels()
handles = [handles[1], handles[0]]#, handles[2]]
labels = ["Lab_Pro", "Lab_Oth"]#, "UKIP"]
plt.legend(handles, labels, loc="lower right", shadow=False, scatterpoints=1, fontsize=8)
plt.xlabel('cPC1')
plt.ylabel('cPC2')
plt.title("cMCA w/ Subgroups (tg: LAB, bg: UKIP, " + str(alpha) + ": 10)")
plt.show()
f_2.savefig("cMCA_ESS2018_labuip_new2.pdf", bbox_inches='tight')
