...
cmca = cmca.fit(fg=X_con.iloc[:,0:(X_con.shape[1]-3)], bg=X_lab.iloc[:,0:(X_lab.shape[1]-3)], alpha=1.5)

Y_fg = np.array(cmca.transform(X_con.iloc[:,0:(X.shape[1]-3)]))
Y_bg = np.array(cmca.transform(X_lab.iloc[:,0:(X.shape[1]-3)]))
...
plt.xlim([-1.5, 4])
plt.ylim([-2.5, 1.5])
plt.scatter(Y_fg[:, 0], Y_fg[:, 1], c=tableau10[X_con["prtclcgb"].iloc[0]], label=party[X_con["prtclcgb"].iloc[0]], alpha=0.3, linewidths=0)
plt.scatter(Y_bg[:, 0], Y_bg[:, 1], c=tableau10[X_lab["prtclcgb"].iloc[0]], label=party[X_lab["prtclcgb"].iloc[0]], alpha=0.3, linewidths=0)
plt.legend(loc="lower right", shadow=False, scatterpoints=1, fontsize=8)
plt.xlabel('cPC1')
plt.ylabel('cPC2')
plt.title("cMCA (tg: CON, bg: LAB, " + str(alpha) + ": 1.5)")
plt.show()
f_5.savefig("cMCA_ESS2018_conlab_org.pdf", bbox_inches='tight')
