...
plt.xlim([-0.5, 1.5])
plt.ylim([-2, 3.5])
plt.scatter(Y_fg[:, 0], Y_fg[:, 1], c=tableau10[X_d["partyid"].iloc[0]], label=party[X_d["partyid"].iloc[0]], alpha=0.8, linewidths=0)
...
plt.xlabel('cPC1')
plt.ylabel('cPC2')
plt.title("cMCA (tg: Dem, bg: Rep, " + str(alpha) + ": 1.5)")
plt.show()
f.savefig("cMCA_CCES2015_demrep_org.pdf", bbox_inches='tight')
