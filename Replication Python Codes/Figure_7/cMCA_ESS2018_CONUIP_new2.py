...
for label in X_con['pro_eu'].unique():
    plt.scatter(Y_fg[X_con['pro_eu'] == label, 0], Y_fg[X_con['pro_eu'] == label, 1], c=tableau10[label], label=label, alpha=0.6, linewidths=0)
#plt.scatter(Y_bg[:, 0], Y_bg[:, 1], c=tableau10[1], label=party[X_uip["prtclcgb"].iloc[0]], alpha=0.8, linewidths=0)
...
plt.show()
f_4.savefig("cMCA_ESS2018_conuip_new2.pdf", bbox_inches='tight')
